...
export function createSimulator(options: SimulatorOptions): Simulator {
  ...
  function tick() {
    state = produce((draft: Draft<State>) => {
      updateMessagesBuffer(draft, messagesBuffer, options);
      updateMessages(draft, nodesMap);
      updateNodes(draft, nodes);
    })(state);
  }
...
function updateNodes(draft: Draft<State>, nodes: Node[]) {
...
function updateMessages(draft: Draft<State>, nodesMap: Record<number, Node>) {
...
function updateMessagesBuffer(
  draft: Draft<State>,
  messagesBuffer: Message[],
  { messageChannelTicks }: SimulatorOptions
) {
